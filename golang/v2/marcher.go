...
func NewMarcher(params *MarchingCubesParams, tables *MarchingCubesTables, noise *Noise3D) Marcher {
	return Marcher{
		params,
		tables,
		noise,
...
func (m *Marcher) March() bool {
	corners := []Vector4{
		m.evaluate(m.cursor.AddV3(Vector3{0, 0, 0})),
		m.evaluate(m.cursor.AddV3(Vector3{1, 0, 0})),
		m.evaluate(m.cursor.AddV3(Vector3{1, 0, 1})),
		m.evaluate(m.cursor.AddV3(Vector3{0, 0, 1})),
		m.evaluate(m.cursor.AddV3(Vector3{0, 1, 0})),
		m.evaluate(m.cursor.AddV3(Vector3{1, 1, 0})),
		m.evaluate(m.cursor.AddV3(Vector3{1, 1, 1})),
		m.evaluate(m.cursor.AddV3(Vector3{0, 1, 1})),
	}
